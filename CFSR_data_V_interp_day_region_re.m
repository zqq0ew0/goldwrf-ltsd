...
%     spd_interp_v=[];   
    ...
    try               
        ps1=data_sfc.data('PRES_SFC'); %地表气压
        disp('ps download successfully')
        v101=data_sfc.data('V_GRD_HTGL'); %10m处V
        disp('v10 download successfully')
        u101=data_sfc.data('U_GRD_HTGL'); %10m处U
...
%     for ntime=1:num_t_prc
%         disp([num2str(year0),' : ',num2str(ntime)])
        
%         ps1=data_sfc.data('PRES_SFC',[ntime,1,1],[ntime,num_lat_sfc,num_lon_sfc]); %地表气压
%         v101=data_sfc.data('V_GRD_HTGL',[ntime,1,1],[ntime,num_lat_sfc,num_lon_sfc]); %10m处V
%         u101=data_sfc.data('U_GRD_HTGL',[ntime,1,1],[ntime,num_lat_sfc,num_lon_sfc]); %10m处U
%         
...
%         z0=data_prc.data('HGT_ISBL',[ntime,1,1,1],[ntime,37,num_lat_prc,num_lon_prc]); %重力势能 压强越大，势能越小 16个等压面数据，从500~1000hpa
%         v0=data_prc.data('V_GRD_ISBL',[ntime,1,1,1],[ntime,37,num_lat_prc,num_lon_prc]);
%         u0=data_prc.data('U_GRD_ISBL',[ntime,1,1,1],[ntime,37,num_lat_prc,num_lon_prc]);
...
%        %% 等压面数据  
%         n_error=1;
%         for i=21:num_lon_prc
%             for j=45:num_lat_prc
%                     %计算u、v
...
%                         lat_prc(j,i)
%                         return
%                         [~,h]=contourf(lon_prc,lat_prc,topo0);
...
%     save([path_out,'mat\matlab_spd_interp_v_',int2str(year0),'.mat'],'spd_interp_v');
    save([path_out,'mat\matlab_ps1_prc_',int2str(year0),'.mat'],'ps1_prc');
    save([path_out,'mat\matlab_u101_prc_',int2str(year0),'.mat'],'u101_prc');
    save([path_out,'mat\matlab_v101_prc_',int2str(year0),'.mat'],'v101_prc');
