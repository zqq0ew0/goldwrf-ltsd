...
path_mat='E:\data\CFSR\raw_data\mat_2019\';
...
for year0=year(2):-1:year(1)
    ...
    load([path_mat,'matlab_datetime_sfc_2019.mat']);
%     load([path_mat,'matlab_datetime_prc_2019.mat']); 
    datetime1=datetime_sfc;
    load([path_mat,'matlab_datetime_sfc_20190828_20190915.mat']);
    datetime2=datetime_sfc+datetime1(end)+6;
    ...
    t2m2=t2m;
    load([path_mat,'matlab_t2m_prc_20190828_20190915.mat']);
    t2m3=t2m_prc;
    load([path_mat,'matlab_rh2m_prc_2019_0.mat']);
    rh2m1=rh2m;  
    load([path_mat,'matlab_rh2m_prc_2019_1.mat']);
    rh2m2=rh2m;  
    load([path_mat,'matlab_rh2m_prc_20190828_20190915.mat']);
    rh2m3=rh2m_prc; 
    
    t2m=cat(1,t2m1,t2m2);
    t2m=cat(1,t2m,t2m3);
    rh2m=cat(1,rh2m1,rh2m2);     
    rh2m=cat(1,rh2m,rh2m3);
 
    load([path_mat,'matlab_ps_sfc_2019.mat']);
    ps1=ps_sfc;
    load([path_mat,'matlab_ps_sfc_20190828_20190915.mat']);
    ps2=ps_sfc;
    load([path_mat,'matlab_u10_sfc_2019.mat']);
    u101=u10_sfc;
    load([path_mat,'matlab_u10_sfc_20190828_20190915.mat']);
    u102=u10_sfc;
    load([path_mat,'matlab_v10_sfc_2019.mat']);    
    v101=v10_sfc;
    load([path_mat,'matlab_v10_sfc_20190828_20190915.mat']);    
    v102=v10_sfc;
    ...
    load([path_mat,'matlab_z0_prc_20190828_20190915.mat']);
    load([path_mat,'matlab_u0_prc_20190828_20190915.mat']);
    load([path_mat,'matlab_v0_prc_20190828_20190915.mat'])    
    
    % process data
    num_t=size(datetime,2);
...
%     end
    ...
    %             disp('z01 wrong data, reshape...')
            ...
            z012=squeeze(z0_prc(ntime-956,:,:,:));
            v012=squeeze(v0_prc(ntime-956,:,:,:));
            u012=squeeze(u0_prc(ntime-956,:,:,:));
        end
       %% 等压面数据       
        for i=1:num_lon_prc
            for j=1:num_lat_prc
